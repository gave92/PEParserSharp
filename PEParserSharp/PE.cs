...
namespace PEParserSharp
{
	...
	using COFFFileHeader = PEParserSharp.headers.COFFFileHeader;
	using Header = PEParserSharp.headers.Header;
	using OptionalHeader = PEParserSharp.headers.OptionalHeader;
	using SectionTable = PEParserSharp.headers.SectionTable;
	using SectionTableEntry = PEParserSharp.headers.SectionTableEntry;
	using ResourceDataEntry = PEParserSharp.headers.resources.ResourceDataEntry;
	using ResourceDirectoryEntry = PEParserSharp.headers.resources.ResourceDirectoryEntry;
	using ResourceDirectoryHeader = PEParserSharp.headers.resources.ResourceDirectoryHeader;
		...
		public virtual string Info
		{
			get
			{
				if (PE)
				{
					...
					foreach (ByteDefinition bd in this.optionalHeader.headers)
					{
						bd.format(b);
					}
					b.Append(System.Environment.NewLine);
